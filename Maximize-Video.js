//让网页中的任何一个视频网页全屏播放！
//支持有多个视频的任意网页(比如贴吧、论坛等)。支持复杂页面多层iframe嵌套的视频。支持HTML5视频。支持flash游戏。
//网页全屏后点浏览器最左边或最右边或按ESC都可以退出全屏。
//如果遇到HTML5播放器不显示网页全屏按钮或别的问题，尝试按ESC键缩放。
// ==UserScript==
// @name                Maximize Video
// @name:zh-CN          视频网页全屏
// @namespace           
...
;(() => {
  ...
  const setButton = {
    ...
    locate() {
      ...
      gv.speedBtn.innerHTML = gv.speedRate + "x"
  ...
  const handle = {
    ...
    setSpeed(rate, showTip) {
      ...
      if (gv.speedBtn) gv.speedBtn.innerHTML = rate + "x"
    ...
    fullWin() {
      if (!gv.isFull) {
        ...
        if (tool.isYoutube() && !document.querySelector("#player-theater-container #movie_player")) {
          const sizeBtn = document.querySelector("#movie_player .ytp-size-button")
          if (sizeBtn) {
            sizeBtn.click()
      ...
      if (getComputedStyle(gv.controlBtn).position != "fixed") {
        tool.addStyle(
          [
            ...
            tool.isBilibili() ? "#htmlToothbrush #bodyToothbrush .parentToothbrush {z-index:999999 !important;}" : "",
